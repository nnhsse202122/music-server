...
class DeleteSongFromPlaylistOverlayModel extends OverlayModelBase {
    ...
    async _onDeleteButtonClicked() {
        ...
        let currentSongReq = await SongServerAPI().classroom(classCode).playlist.currentSong.get();
        if (req.success) {
            let valid = false;
            let songs;
            let songData;
...
class DeleteStudentOverlayModel extends OverlayModelBase {
    ...
    async _onDeleteButtonClicked() {
        this.overlay.show("loading");
        let response = await SongServerAPI().classroom(classCode).students.find(this.getData("email")).remove();
        if (!response.success) {
            this.overlay.hide();
            throw new Error(response.message);
        }
...
class DeleteClassOverlayModel extends OverlayModelBase {
    ...
    async _onDeleteButtonClicked() {
        this.overlay.show("loading");
        let req = await SongServerAPI().classroom(classCode).delete();
        let currentSongReq = await SongServerAPI().classroom(classCode).playlist.currentSong.get();
