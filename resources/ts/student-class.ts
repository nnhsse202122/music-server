...
// @ts-ignore
function createPlaylistItem(container: HTMLElement, position: number, title: string, liked: boolean) {
...
function refreshPlaylist() {
    SongServerAPI().classrooms.find(classCode).settings.get().then((sData) => {
        let settingsData: SongServer.API.Responses.ClassroomSettingsResponse;
        if (!sData.success) {
